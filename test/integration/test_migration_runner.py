...
# test of migration from v 0.1.4. to 0.2.3
class TestMigrationsResolver(unittest.TestCase):
    ...
    def test_migration_table_is_created(self):
        try:
            ...
            # migration version
            migration.main(
                ["-u", SQLALCHEMY_URL, "-s", DATA_QUALITY_SCHEMA, "-v", "0.2.3", "-f"]
            )
    ...
    def test_migration_to_0_2_3(self):
        try:
            # we must use -f (force) in the case that contessa.__version__ possibly do not match our testing
            # migration version
            migration.main(
...
# test of migration from v 0.1.4. to 0.2.3 transaction
class TestMigrationsResolverTransaction(unittest.TestCase):
    ...
    def test_migration_to_0_2_3_transaction(self):
        try:
            migration.main(
                ["-u", SQLALCHEMY_URL, "-s", DATA_QUALITY_SCHEMA, "-v", "0.2.3"]
            )
